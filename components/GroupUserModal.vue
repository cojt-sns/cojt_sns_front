...
export default {
  ...
  data() {
    return {
      ...
      image: process.env.SERVER_URL + this.groupUser.image,
      imageEdit: false,
      error: '',
  ...
  watch: {
    open(newValue) {
      if (newValue) {
        ...
        this.image = process.env.SERVER_URL + this.groupUser.image;
        this.error = '';
  ...
  methods: {
    async edit() {
      ...
      try {
        if (this.myIcon) {
          await GroupUser.putGroupUser(this.groupUser.id, this.name, this.file);
        } else {
